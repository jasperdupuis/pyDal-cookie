...
fig,ax = plt.subplots()
for index in range (len(y_values))    :
    ax.scatter(f[:10000],s_std[index],label=str(y_values[index]),marker='.')
ax.set_xscale('log')
ax.set_xlim((0,500))
ax.set_xlabel('Frequency (Hz)',fontsize=_thesis.SIZE_AX_LABELS)
ax.set_ylabel(r'Standard deviation, dB ref $1{\mu}Pa^2 / Hz$',fontsize=_thesis.SIZE_AX_LABELS)
plt.savefig(fname = _dirs.DIR_RESULT_NOISEBAND  \
            + figname +'.eps',
            bbox_inches='tight',
            format='eps',
            dpi = _thesis.DPI)    
